...
import { getInviteUsers, inviteUser, type InviteUser } from "@/api/room";
...
export default function InviteLearnersModal({
  ...
  const handleInvite = async (userId: string) => {
    if (invited.has(userId) || inviting.has(userId)) return;
            ...
            <motion.div
              ...
              {/* <div className="mx-6 mb-4 flex items-center gap-3 bg-blue-50 border border-blue-100 rounded-2xl px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[14px] font-black text-[#1C398E] tracking-widest">
                    {ROOM_CODE}
                  </p>
                </div>
                <button
              ...
              <div className="px-6 pb-2 max-h-64 overflow-y-auto space-y-1 hideScrollbar">
                {loading ? (
                  ...
                  filtered.map((u) => {
                    ...
                    const isInviting = inviting.has(u.id);
                    return (
                      <div
                        key={u.id}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {(u.level != null || u.subject) && (
                            <p className="text-[11px] text-gray-400 font-medium">
                              {u.level != null && `LVL ${u.level}`}
                              {u.level != null && u.subject && " · "}
                              {u.subject}
                            </p>
                          )}
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 text-[11px] font-black uppercase tracking-wider px-3 py-1.5 rounded-xl transition-colors focus-visible:outline-none disabled:cursor-not-allowed ${
                            isInvited
