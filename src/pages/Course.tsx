...
const Course = () => {
  ...
  const course = (courses as CourseType[]).find((c) => c.name.toLowerCase() === name);
      ...
      <div className="py-12">
        <div className="p-6 pb-24 md:pb-6 md:pl-28">
          ...
          <div className="flex items-center gap-4 mb-8">
            <div className={`${course.color} p-4 rounded-2xl text-white`}>{Icon && <Icon size={24} />}</div>

            <div>
              <h2 className="text-2xl font-bold text-gray-900">{course.name}</h2>
              <p className="text-gray-500">{course.topics.length} topics available</p>
            </div>
          </div>
          ...
          <div className="grid gap-4">
            {course.topics.map((topic, index) => (
              <div
                ...
                onClick={() => navigate(`/courses/${course.slug}/${topic.slug}`)}>
